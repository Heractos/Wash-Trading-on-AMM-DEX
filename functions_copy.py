...
# get pair info
def get_pair_info(
    pair_address, pair_contract_ABI, eth_scan_api_key, w3=None, token_abi=None
):
...
def find_other_txs_and_analyze_them(all_txs_df, target_address, pair_info):
    ...
    wash_trading_data = data[(np.abs(stats.zscore(data["swap_size"])) < 3)]
    display(data[(np.abs(stats.zscore(data["swap_size"])) >= 3)])
